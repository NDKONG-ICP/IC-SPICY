...
export const auditLog = {
  log(action, userId, details = {}) {
    ...
    // Store locally for debugging and transaction history
    const logs = JSON.parse(localStorage.getItem('audit_logs') || '[]');
    logs.push(logEntry);
    
    // Keep only last 1000 logs for better performance
    if (logs.length > 1000) {
    ...
    // Also store in a separate transaction log for the Portal page
    const transactionLogs = JSON.parse(localStorage.getItem('transaction_logs') || '[]');
    if (this.isTransactionAction(action)) {
      transactionLogs.push({
        ...logEntry,
        tx_type: this.getTransactionType(action),
        amount: details.amount || 0,
        token: details.token || 'UNKNOWN',
        from: details.from || userId,
        to: details.to || 'SYSTEM'
      });
      
      // Keep only last 500 transaction logs
      if (transactionLogs.length > 500) {
        transactionLogs.splice(0, transactionLogs.length - 500);
      }
  ...
  // Check if an action is a transaction
  isTransactionAction(action) {
    const transactionActions = [
      'stake', 'unstake', 'claim_rewards', 'transfer', 'mint', 'burn',
      'purchase_product', 'tip_user', 'join_membership', 'vote_proposal',
      'create_proposal', 'harvest_plant', 'plant_seed', 'water_plant'
    ];
    return transactionActions.includes(action);
  },

  // Get transaction type from action
  ...
  cleanupOldLogs() {
    ...
    const filteredTransactionLogs = transactionLogs.filter(log => new Date(log.timestamp) > thirtyDaysAgo);
    
    localStorage.setItem('audit_logs', JSON.stringify(filteredLogs));
    localStorage.setItem('transaction_logs', JSON.stringify(filteredTransactionLogs));
  },
  ...
  exportLogs(type = 'all') {
    if (type === 'transactions') {
      ...
      return {
        ...
        transactions: this.getTransactionLogs()
      };
    }
  }
}; 
