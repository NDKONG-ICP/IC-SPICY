...
const AdminPage = () => {
  ...
  const loadAdminData = async () => {
    ...
    try {
      ...
      // Load treasury balances and cross-chain payments
      const treasuryBalances = {};
      let crossChainPayments = [];
      try {
        // Load individual treasury balances
        ...
        // Load cross-chain payments (includes IcPay transactions)
        crossChainPayments = await membershipActor.list_cross_chain_payments();
        console.log('📊 Loaded cross-chain payments:', crossChainPayments);
      } catch (e) {
        console.warn('Failed to load some treasury balances or cross-chain payments:', e);
      }
      ...
      <div className="space-y-8">
        ...
        {activeTab === 'analytics' && (
          <div className="space-y-6">
            ...
            {/* Cross-Chain Payments (IcPay Transactions) */}
            <div className="glass-card-dark p-6 border border-blue-500/30">
              <h3 className="text-xl font-bold text-white mb-4">Recent Cross-Chain Payments</h3>
              
              {analytics.crossChainPayments.length === 0 ? (
                <div className="text-center text-gray-400 py-8">
                  <div className="text-4xl mb-2">💳</div>
                  <p>No cross-chain payments found</p>
                </div>
              ) : (
                <div className="space-y-3 max-h-96 overflow-y-auto">
                  {analytics.crossChainPayments.slice(-10).reverse().map((payment, index) => (
                    <div key={payment.id} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-400">
                              {payment.membership_tier} • {payment.user_principal.substring(0, 8)}...
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-gray-400">
                          {new Date(Number(payment.created_at) / 1000000).toLocaleDateString()}
                        </div>
