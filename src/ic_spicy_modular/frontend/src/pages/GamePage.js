...
import { recordGameTransaction } from '../services/TransactionService';
...
const GamePage = () => {
  ...
  const [demoMode, setDemoMode] = useState(false); // Use real data by default
      ...
      if (success) {
        ...
        try {
          await recordGameTransaction({
            ...
            amount: amount,
            currency: 'SPICY',
            transactionHash: `game_spend_spicy_${Date.now()}`,
            metadata: {
              type: 'spend_spicy',
              amount: amount,
          ...
          console.warn('⚠️ Failed to record game transaction in admin analytics:', analyticsError);
        }
      } else {
    ...
    try {
      ...
      if (result.success) {
        ...
        try {
          await recordGameTransaction({
            userPrincipal: principal.toString(),
            amount: 10, // Cost to play level
            currency: 'SPICY',
            transactionHash: `game_play_level_${Date.now()}`,
            metadata: {
              type: 'play_level',
              cost: 10,
              heatEarned: result.heatEarned,
              coinsEarned: result.coinsEarned,
              newSpicyBalance: Number(newSpicyBalance),
              newHeatBalance: Number(newHeatBalance),
              action: 'level_completion'
            }
          });
        } catch (analyticsError) {
          console.warn('⚠️ Failed to record game transaction in admin analytics:', analyticsError);
